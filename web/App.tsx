...
import { AnimationInfo, animationInfo as getAnimationInfo } from "./imageUtils";
...
const FORM_ELEMENTS = {
	...
	customWidth: {
		...
		max: 8000,
		tooltip: `
		Sets a custom width (in pixels) for the the lamps.
		`,
	},
	customHeight: {
		name: "Height",
		...
		max: 8000,
		tooltip: `
			Sets a custom height (in pixels) for the the lamps.
		`,
	},
	substationQuality: {
...
function App({ worker }: { worker: Worker }) {
	...
	const [animationInfo, setAnimationInfo] = createSignal<AnimationInfo>();
	...
	// Refs
	const formRefs: {
		blueprintResult: HTMLElement;
		blueprintStatus: HTMLElement;
	...
	async function handleSubmit(event: SubmitEvent) {
		...
		try {
			const imageData = formData.file ? new Uint8Array(await formData.file.arrayBuffer()) : new Uint8Array();
				...
				<div ref={form!} class="panel-container flex">
					<div classList={{ hidden: isGenerating() }} class="panel form flex-shrink-0">
						...
						<form onSubmit={handleSubmit} class="panel-inset-light bg-gray-500 p-6 rounded shadow-md w-full max-w-md">
									...
									<div>
										<div class="text-gray-300">
											<div class="flex items-center justify-between">
												...
												<div>
													<span class="font-semibold">{info.width}</span> x{" "}
													<span class="font-semibold">{info.height}</span> px
												</div>
											</div>
											<div class="flex items-center justify-between">
												<div>
							...
							{/* Max Size Input */}
							<div
								class="mt-5 mb-4 flex items-center justify-between"
								classList={{
									"pointer-events-none": formData.mode !== "full",
									"opacity-50": formData.mode !== "full",
								}}
							>
						...
						>
							{Object.entries(FORM_ELEMENTS).map(([k, v]) => {
								if (k === "substationQuality") {
									...
									return null;
								} else if (formData.mode === "full" && (k === "customWidth" || k === "customHeight")) {
									return null;
								}
...
type FormElementValue = { disabled?: boolean; name: string; tooltip?: string | null | undefined; type: string };
...
function makeFormElement({
	...
	formRefs: Record<string, HTMLElement>;
	setFormData(key: string, value: any): void;
	key: string;
	value: FormElementValue;
}) {
	const { disabled = false, name, tooltip = null, type } = value;

	function mkTooltip(text: string | null | undefined) {
		if (!text) return null;
	...
	if (type === "checkbox") {
			...
			<div class="mb-1 flex">
				<label class="checkbox-label">
					<input
						...
						onChange={(e) => setFormData(key, e.currentTarget.checked)}
					/>
		...
		const { options } = value as unknown as { options: string[][] | Record<string, string> };
		...
		return (
			<div class="mt-1 mb-1 flex items-center justify-between factorio-select-container">
				...
				>
					<For each={Array.isArray(options) ? options : Object.entries(options)}>
		...
		function handleSubmit(rawValue: number) {
			const ref = formRefs[key] as HTMLInputElement;

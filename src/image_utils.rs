...
// /// (Optimised) Finds the number of frames in a GIF image.
// ///
// /// # Arguments
// ///
// /// * `data` - The image data.
...
// /// # Returns
// ///
// /// The number of frames in the image, or an error if the data is invalid.
// pub fn count_gif_frames(data: &[u8]) -> Result<u32, &'static str> {
//     if data.len() < 13 {
...
//     }
//     let packed = data.get(10).copied().ok_or("Truncated GIF")?;
//     let mut pos = 13;
...
//             0x2C => {
//                 n_frames = n_frames.checked_add(1).ok_or("Too many GIF frames")?;
//                 if data.len().saturating_sub(pos) < 9 {
//                     return Err("Truncated image descriptor"); // Image Descriptor: left (2) top (2) width (2) height (2) packed (1)
//                 }
...
//                 }
//                 pos += 1;
//                 skip_sub_blocks(data, &mut pos)?; // LZW data sub-blocks
//             }
//             // Extension block
//             0x21 => {
//                 if data.get(pos).is_none() {
//                     return Err("Truncated extension"); // Extension label
//                 }
//                 pos += 1;
//                 skip_sub_blocks(data, &mut pos)?; // Extension data is a series of sub-blocks
//             }
//             0x3B => {
//                 return Ok(n_frames); // GIF trailer
//             }
//             _ => {
...
// /// (Optimised) Finds the number of frames in a WebP image.
// ///
// /// # Arguments
// ///
// /// * `data` - The image data.
...
// ///
// /// The number of frames in the image, or an error if the data is invalid.
// pub fn count_webp_frames(data: &[u8]) -> Result<u32, &'static str> {
//     if data.len() < 12 {
...
//     let end = 8usize.checked_add(riff_size).ok_or("Invalid RIFF size")?; // RIFF size excludes the 8-byte "RIFF" + size header.
//     let mut pos = 12;
...
//             frame_count = frame_count.checked_add(1).ok_or("Too many WebP frames")?;
//         }
//         pos += chunk_size; // RIFF chunks are padded to an even number of bytes.
//         if chunk_size & 1 != 0 {
//             pos += 1;
//         }
//     }
